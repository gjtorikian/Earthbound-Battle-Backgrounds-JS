(function (window, document) {
  window.anigif_bar = {
    resolve_mode:
      typeof anigif_base_url == "undefined"
        ? "absolute_from_root"
        : "absolute_from_remote",

    base_url:
      typeof anigif_base_url == "undefined"
        ? getAbsoluteUrlPrefix() + "anigif/"
        : anigif_base_url,
    ...
    install: function () {
      ...
      this.downloadHtml(self.base_url + "bar.html", function (err, html) {
        var div = document.getElementById("gifMakerContainer");
    ...
    downloadHtml: function (url, cba) {
      ...
      xmlHttp.onreadystatechange = function () {
        ...
        if (xmlHttp.readyState == 4) {
          cba(
            xmlHttp.status == 200 ? null : xmlHttp.status,
    ...
    init: function (el) {
      ...
      this.status("GIF recorder");

      for (var i = 0; i < self.buttons.length; i++) {
        this.el.querySelectorAll("#" + self.buttons[i])[0].onclick = function (
          e
        ) {
    ...
    loadConfig: function () {
      ...
      this.el.querySelectorAll("#onlyLastFrames")[0].value =
        window.anigif.options.onlyLastFrames;
      this.el.querySelectorAll("#framesPerSecond")[0].value =
        window.anigif.options.framesPerSecond;
      this.el.querySelectorAll("#rootNode")[0].value =
        window.anigif.options.selector;
      this.el.querySelectorAll("#ratio")[0].value = window.anigif.options.ratio;
      this.el.querySelectorAll("#quality")[0].value =
        window.anigif.options.quality;
      this.el.querySelectorAll("#period")[0].value =
        window.anigif.options.period;
      this.el.querySelectorAll("#fixedWidth")[0].value =
        window.anigif.options.fixedWidth;
    },
    ...
    checkBrowser: function () {
      ...
      if (!isChrome & !isFirefox & !isSafari) {
        alert(
          "Gifw00t! has been tested on Chrome, Firefox and Safari only and may not work on your browser"
        );
      }
    },
    ...
    setEnabled: function (settings) {
      for (var b in settings) {
        ...
        this.el.querySelectorAll("#" + b)[0].className = settings[b]
          ? "enabled"
          : "disabled";
      }
    },
